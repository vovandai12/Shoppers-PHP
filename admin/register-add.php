...
if (strlen($_SESSION['admin']) == 0) {
    ...
    if ($query->rowCount() > 0) {
        foreach ($results as $result) {    ?>
            ...
            $ddecentralization = htmlentities($result->decentralization);
            ...
            if (isset($_POST['Update'])) {
                ...
                $email = $_POST['email'];
                $pass = $_POST['pass'];
                $decentralization = $_POST['decentralization'];
                $sql = "UPDATE `admin` SET `user`=:user,`email`=:email,`pass`=:pass, `decentralization`=:decentralization WHERE `id`=:id";
                $query = $dbh->prepare($sql);
                $query->bindParam(':user', $user, PDO::PARAM_STR);
                $query->bindParam(':email', $email, PDO::PARAM_STR);
                $query->bindParam(':pass', $pass, PDO::PARAM_STR);
                $query->bindParam(':decentralization', $decentralization, PDO::PARAM_STR);
                $query->bindParam(':id', $id, PDO::PARAM_STR);
                $query->execute();
            ...
            if (isset($_POST['Save'])) {
                ...
                $pass = $_POST['pass'];
                $decentralization = $_POST['decentralization'];
                $sql = "INSERT INTO `admin`(`username`, `email`, `pass`, `decentralization`) VALUES (:user, :email, :pass, :decentralization)";
                $query = $dbh->prepare($sql);
                $query->bindParam(':user', $user, PDO::PARAM_STR);
                $query->bindParam(':email', $email, PDO::PARAM_STR);
                $query->bindParam(':pass', $pass, PDO::PARAM_STR);
            ...
            <form action="" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="decentralization" class="form-control" placeholder="Enter Decentralization" value="<?php echo $ddecentralization ?>">
                    </div>
